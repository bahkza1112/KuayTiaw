...
### Changed
- `assets/images/tower_sniper.png`, `tower_support.png` (+ ทุกขั้นดาว s2/s3/s4 และคู่ `_g2.png`, รวม 16
  ไฟล์): เจนใหม่ทั้งหมด — เดิมสองป้อมนี้หลุดโทนจากป้อมอื่น (สไนเปอร์เรียบง่ายกว่าตัวอื่นมาก, ซัพพอร์ตธีมไม้เลื้อย
  ต่างจากธีม "หินสะอาด + คริสตัลเรืองแสง" ของป้อมอื่น) ตอนนี้ทั้งคู่ใช้ธีม "หิน + คริสตัลเรืองแสง" เข้าชุดกับ
  ป้อมอื่นๆ ทั้งหมดแล้ว — รัน `fix_tower_scale.py`/`gen_glow_frames.py` ซ้ำให้ตรงกับภาพใหม่ด้วย

### Added
- `assets/images/enemy_*_g2.png` (19 ไฟล์ — ทุกมอนสเตอร์): เฟรม B แบบเดียวกับป้อม (boost แสง/สีจุดเรือง
  แสงจากภาพเดิม ไม่เจนใหม่ผ่าน AI) — `js/enemy.js` `_enemyImg2(ti)` + `drawEnemySprite` crossfade ทับ
  เฟรม A ด้วย sin-wave ต่อเนื่อง (phase offset ต่อชนิดมอน) ให้มอนสเตอร์มีจุดเรืองแสง/ดวงตาไหวตอนเดิน
  เหมือนที่ป้อมมีอยู่แล้ว (v3.25.1)

## v3.25.3 — ปรับขนาดป้อม/มอนสเตอร์ให้พอดีขึ้น

### Changed
- `js/tower.js` `drawTowerIcon`: ลดขนาดวาดป้อมจาก `sz*1.62` → `sz*1.42` — ป้อมที่วางในช่องติดกันเคย
  ทับกันเองด้วยภาพที่ใหญ่เกินช่อง ตอนนี้แต่ละป้อมเห็นแยกจากกันชัดขึ้น (ไม่กระทบ hitbox/ระยะยิง เป็นแค่
  ขนาดภาพที่วาด)
- `js/enemy.js` `ESIZES`: ลดขนาดมอนสเตอร์ทั่วไป (non-boss, `MTYPE!==1`) ลง ~12% — บอสทั้ง 5 ตัว (index
  4/9/14/15/16 ตาม `MTYPE`) ขนาดคงเดิม ยังคงความรู้สึก "ใหญ่อลังการ" ต่างจากมอนสเตอร์ทั่วไปชัดเจนขึ้นไปอีก
  (ไม่กระทบ HP/ดาเมจ/hitbox เป็นแค่ขนาดภาพที่วาด)

## v3.25.2 — แก้ป้อมขั้นสูงดูลอย + เพิ่มออร่าอลังการตามดาว

### Fixed
- `assets/images/tower_*_s{2,3,4}.png` และคู่ `_g2.png` (60 ไฟล์): ภาพวิวัฒนาการที่เจนแยกจากภาพ 1★ เดิม
  ถูก crop/trim อิสระต่อกัน ทำให้แต่ละขั้นดาว "เนื้อภาพเต็มเฟรม" ไม่เท่ากัน (เช่น cannon 1★ เนื้อภาพเต็ม
  83% ของเฟรม แต่ cannon_s4 เต็ม 90%) เมื่อวาดด้วยกล่องขนาดคงที่เดียวกันทุกขั้น ทำให้ป้อมขั้นสูงดูใหญ่/ลอย
  เกินเงาที่ตรึงตามขนาดกริดเกม ไม่ใช่ตามตัวภาพ — แก้ด้วย `fix_tower_scale.py` (ปรับสเกลภาพแต่ละขั้นให้
  สัดส่วนเนื้อภาพต่อเฟรมตรงกับภาพ 1★ ของป้อมชนิดนั้นๆ) ตอนนี้ทุกขั้นดาวยืนบนเส้นพื้นเดียวกัน

### Added
- `js/tower.js` `_twAura(ctx,type,r,star)`: เพิ่มพารามิเตอร์ `star` — ออร่าใต้เท้าโต/สว่างขึ้นตามขั้นดาว
  (2★→3★→4★ โต 11%/22%/33% + พัลส์แรงขึ้น) และป้อม 4★ (ขั้นสุด) มีประกายดาวทอง 4 จุดโคจรรอบฐานเพิ่ม
  ความอลังการสมกับเป็นขั้นสูงสุด
...
### Changed
- `js/game.js` `_playSound` case `'archer'` (ใช้กับป้อมมินิกัน type5 ผ่าน `_TSND`): เปลี่ยนจากเสียงสาย
  ธนูดีด (twang) เป็นเสียงปืนกลโลหะแบบกลไกจริง — bandpass noise crack แหลม (เสียงปัง), square-wave
  thump ความถี่ต่ำไล่ลง (กระแทกลูกเลื่อน/ห้องกระสุน), และเสียงกริ๊งโลหะทองเหลืองแหลมสั้นๆ ท้ายเสียง
  (ปลอกกระสุนกระเด็น) — ปรับ `_bodyWarmth` ให้กระด้าง/โลหะขึ้นให้เข้ากับโทนใหม่
...
### Changed
- `js/game.js` `STAGES[11..19]`: ใส่ `startGold` ให้ด่าน Act 2 ไต่ระดับสม่ำเสมอ (11:190, 12:200, 13:210, 14:230, 15:225, 16:220*, 17:245, 18:255, 19:265, 20:280*) จากเดิมหลายด่านใช้ค่า default 175 (เท่าด่านแรกของเกม) — วิเคราะห์: HP มอน scale ตามด่าน (×(1+si×0.27)) แต่ reward/kill คงที่ (curve ต่อเนื่องจาก Act 1) การเพิ่ม startGold ช่วยทุนตั้งตัวช่วงต้นของด่านโหดโดยไม่แตะ curve HP/reward (*=มีอยู่เดิม)
...
### Added
- `js/ui.js` `STORY_MISSIONS`: เพิ่ม per-stage missions Act 2 อีก 10 รายการ (si 11-20 = ด่าน 12-21: ทะเลทรายเถ้าถ่าน → แกนโลก) รางวัลทอง/เจม/ตั๋วไล่ต่อจาก Act 1 (5,500/850/50 → 13,000/2,000/100)
- เพิ่มไมล์สโตน `m9` (ผ่าน 9 ด่าน) และ `m15` (ผ่าน 15 ด่าน)

### Changed
- `js/ui.js` `STORY_MISSIONS`: อัปเดต `mall`/`mall3s` (ผ่านครบทุกด่าน / ทุกด่าน 3 ดาว) `need` 11 → 21 ให้ครอบคลุมทั้ง 21 ด่าน + bump รางวัลเป็น 8,000/1,500/80 และ 8,000/2,500/100 (ของที่เคลมไปแล้วไม่กระทบ เช็คด้วย claimed set)
...
### Fixed
- `js/game.js`: toast แนะนำ ⛏️ ขุด ตอนเข้าด่าน 1 เดิมเด้งให้ผู้เล่นใหม่ทับกับการสอน (`_TUT_STEPS` ที่สอนขุดอยู่แล้ว) เพิ่มเงื่อนไข `tq_tut_done` ให้เด้งเฉพาะผู้เล่นเก่าที่ผ่าน/ข้ามการสอนแล้ว (เป็น hint สำรอง)
...
### Fixed
- `js/ui.js` `_renderTour`/`_positionTour`: ไฮไลต์เมนูทัวร์เดิมวัดตำแหน่งปุ่มครั้งเดียว (มิด-แอนิเมชัน `.screen-enter` ที่ `showScreen` รีสตาร์ททุกครั้ง / ก่อน layout นิ่ง) → กรอบค้างผิดตำแหน่ง เปลี่ยนเป็น track ตำแหน่งเป้าหมายทุกเฟรมด้วย `requestAnimationFrame` และวัดความสูงกล่องจริงแทนค่าคงที่ 190
- `Tower Quest 🏰.html`: bump cache-bust `?v=` ของ css/js ทั้งหมดจาก `3.21.0` → `3.24.24` (ค้างมาตั้งแต่ 3.21.0 ทำให้ผู้เล่นไม่เคยได้รับอัปเดต js/css ใหม่)

### Changed
- `js/ui.js` `MENU_TOUR_STEPS`/`_tourNext`: ทุกสเต็ปไฮไลต์ปุ่มบนหน้าเมนูในที่ (เดิมเปิดหน้าย่อยด้วย `open:` ซึ่งซ่อน `#mm` ทำให้ปุ่ม nav ที่จะไฮไลต์หายไป) — ครอบคลุมทุกปุ่ม ไม่สลับหน้า
- `css/main.css`: ปุ่มข้าม `.tour-skip` เด่นชัดกลางล่างจอ (pill โปร่งแสง + hover แดง) แทนข้อความเทาเล็กมุมจอ
